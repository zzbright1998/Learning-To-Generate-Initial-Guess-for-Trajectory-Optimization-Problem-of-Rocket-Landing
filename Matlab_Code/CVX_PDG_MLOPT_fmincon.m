...
rng(98);
load('NN_xi.mat');
r1_range = [NN_xi(1), NN_xi(1)];%[1500, 1500];%[1250, 1750];
r2_range = [NN_xi(2), NN_xi(2)];%[0,0];%[-100, 100];
r3_range = [NN_xi(3), NN_xi(3)];%[2000,2000];%[1750, 2250];
...
%prev_shaped_eta = shaped_Eta_time(1:4,:);
load('NN_guess_i.mat');
prev_shaped_eta = NN_guess_i';
prev_eta = reshape(prev_shaped_eta,[4*N,1]);
x0 = 1*prev_eta;
qopt = optimset('MaxFunEvals', 500000, ...
    'maxIter', 4*50, ...
    'Display', 'iter', ...
    'Algorithm','sqp','TolFun',0.05);%,'OutputFcn',@outfun);%interior-point
Eta_Optimal = fmincon(@(x)Omega*x,x0,A_INEQ_FINAL,B_INEQ_FINAL,Aeq_Ter,Beq_Ter,[],[],@nonlcon,qopt);
toc
%pause
...
% Assuming testtraceDNN4128 is already loaded in the workspace
load('testtraceDNN4128.mat')
% Assuming testtraceDNN4128 is already loaded in the workspace
...
total_rows = size(testtraceDNN4128, 1);

% Get the number of blocks
num_blocks = total_rows / block_size;

% Validate that the rows are a multiple of the block size
if mod(total_rows, block_size) ~= 0
    error('Total rows must be a multiple of the block size (83).');
end
...
for block_idx = 1:num_blocks
    start_row = (block_idx - 1) * block_size + 1;
    end_row = block_idx * block_size;
    y = testtraceDNN4128(start_row:end_row, 1);
    plot(y, 'LineWidth', 1.5, 'Color', colors(block_idx, :));
end
title('Column 1');
ylabel('Values');
grid on;
...
for block_idx = 1:num_blocks
    start_row = (block_idx - 1) * block_size + 1;
    end_row = block_idx * block_size;
    y = testtraceDNN4128(start_row:end_row, 2);
    plot(y, 'LineWidth', 1.5, 'Color', colors(block_idx, :));
end
title('Column 2');
ylabel('Values');
...
for block_idx = 1:num_blocks
    start_row = (block_idx - 1) * block_size + 1;
    end_row = block_idx * block_size;
    y = testtraceDNN4128(start_row:end_row, 3);
    plot(y, 'LineWidth', 1.5, 'Color', colors(block_idx, :));
end
title('Column 3');
ylabel('Values');
...
for block_idx = 1:num_blocks
    start_row = (block_idx - 1) * block_size + 1;
    end_row = block_idx * block_size;
    y = testtraceDNN4128(start_row:end_row, 4);
    plot(y, 'LineWidth', 1.5, 'Color', colors(block_idx, :));
end
title('Column 4');
ylabel('Values');
...
% Adjust layout
sgtitle('2D Line Plots for Columns 1, 2, 3, and 4 by Blocks');
